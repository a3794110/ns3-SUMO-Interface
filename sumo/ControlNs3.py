        ...
        if s == simTime :  #output time
            for id1 in trajectoryList :
                ...
                if cnt == (max_num_veh+1):
                    ...
                    subprocess.call(["bash",sumohome+"/tools/RunNs3.sh","simTime" , str(simTime), "enbTxPowerDbm", str(enbTxPowerDbm), "packetSize", str(packetSize), "packetsInterval", str(packetsInterval), "fadingModel", str(fadingModel)])
                    sys.stdout.flush()
                    traci.close()
                    break
            #print(len(Vehicle))
